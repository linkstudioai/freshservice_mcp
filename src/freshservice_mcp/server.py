...
async def filter_tickets(query: str, page: int = 1, workspace_id: Optional[int] = None) -> Dict[str, Any]:
    """You can filter Freshservice tickets using system and custom fields by passing a URL-encoded `query` string (max 512 characters, must be WRAPPED in double quotes). Supported fields: workspace_id, requester_id, email, agent_id, group_id, priority, status, impact, urgency, tag, due_by, fr_due_by, created_at, updated_at, plus any custom ticket fields in snake_case. You can combine conditions with AND, OR, and parentheses. Supported operators include equality (priority:1), greater/less for numbers and dates (priority:>1, created_at:<'2025-09-05'), and null for unassigned fields. Strings go in single quotes, numbers are plain, and dates use UTC YYYY-MM-DD format with only :>/:<. Each page returns 30 results (use page=N for more), and you can set workspace_id to filter across workspaces (0 = all). Status codes: open(2), pending(3), resolved(4) and closed(5). Priority codes: low(1), medium(2), high(3), urgent(4)"""
